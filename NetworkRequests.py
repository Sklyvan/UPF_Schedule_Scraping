...
class BeautifulSoup_URL:
    ...
    def getJSON(self, exportFile=False): # Como al final nos devuelve un horario, lo extraemos en formato JSON ya que es mucho más cómo de trabajar.
        jsonFile = json.loads(self.HTML_Page.text)
        if exportFile: open('Data.json', 'w').write(str(jsonFile))
        return jsonFile